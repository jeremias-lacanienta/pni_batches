#!/bin/bash
# Deploy Passage Migration Lambda to DEV environment (us-east-1)

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FUNCTION_NAME="passage-migration-dev"
REGION="us-east-1"
ZIP_FILE="passage-migration-dev.zip"
...
log "🚀 Deploying Lambda to DEV environment (us-east-1)..."
...
aws events put-rule \
    --name "passage-migration-schedule-dev" \
    --schedule-expression "rate(10 minutes)" \
    --state "DISABLED" \
    --description "Passage migration every 10 minutes (DEV)" \
    --region "$REGION" \
    --output table

# Add Lambda target
aws events put-targets \
    --rule "passage-migration-schedule-dev" \
...
aws lambda add-permission \
    ...
    --source-arn "arn:aws:events:$REGION:$(aws sts get-caller-identity --query Account --output text):rule/passage-migration-schedule-dev" \
